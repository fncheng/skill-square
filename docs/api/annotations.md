...
批注用于给学习笔记、解决方案、杂谈或模型回答中的一段原文补充 Markdown 内容。批注与正文分别保存，不会修改原始 Markdown。
...
| 参数 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `resourceType` | `NOTE \| SOLUTION \| MISCELLANY \| MODEL_RESPONSE` | 是 | 文档类型；`MODEL_RESPONSE` 必须有管理员会话 |
| `resourceId` | `string` | 是 | Markdown 内容 UUID |
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `resourceType` | `NOTE \| SOLUTION \| MISCELLANY \| MODEL_RESPONSE` | 是 | 文档类型；模型回答需管理员会话 |
...
删除笔记、解决方案、杂谈或模型回答时，其批注会由数据库外键级联删除。模型回答相关的读取与写入均要求管理员会话。
