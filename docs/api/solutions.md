...
| `tags` | `string[]` | 是 | 标签数组，去除首尾空白并去重后至少 `1` 个、最多 `20` 个，单个最大长度 `80` |
...
请求体为 `ContentTransferFile`，且 `resourceType` 必须为 `SOLUTION`。v1 文件允许 `resource.tags` 为空，以兼容历史无标签内容；v2 文件的标签在去除首尾空白并去重后至少包含一项。
