...
- [分类接口](./categories.md)
- [标签接口](./tags.md)
- [内容标签词云接口](./content-tags.md)
- [解决方案接口](./solutions.md)
- [学习笔记接口](./notes.md)
- [杂谈接口](./miscellanies.md)
- [模型回答接口](./model-responses.md)
...
- 访客可以调用公开资源的读取接口；模型回答及其导出、相关批注查询均要求管理员会话。
...
- 删除学习笔记、解决方案或杂谈后，其 Markdown 批注会被级联删除。
- 学习笔记、解决方案和杂谈至少包含一个标签；内容标签词云按忽略大小写的标签名称聚合三类公开资源。
- UI 原型以单文件 HTML 保存，预览由前端沙箱隔离，默认阻止外部资源。
- 模型回答为独立私有资源，保存可选来源产品、模型名称与原始 Prompt；不进入公开标签词云和标签内容发现。
