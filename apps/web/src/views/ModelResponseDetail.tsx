...
export function ModelResponseDetail() {
  ...
  const { html, headings } = useMarkdown(response?.content ?? '');
  ...
  const annotationButton = (
    <Button variant="outline" size="sm" onClick={() => surfaceRef.current?.openList()}>
      <MessageSquareText className="h-4 w-4" />批注
