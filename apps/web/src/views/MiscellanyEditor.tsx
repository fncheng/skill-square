...
export function MiscellanyEditor() {
  ...
  return <section><PageHead title={isEdit ? '编辑杂谈' : '新建杂谈'} subtitle="维护标题、摘要、分类、标签与 Markdown 正文内容。" back={isEdit && id ? `/miscellanies/${id}` : '/miscellanies'} actions={<><Button type="button" variant="outline" onClick={() => setFullscreen(true)}><Maximize2 className="h-4 w-4" />全屏双栏</Button><Button disabled={saving} onClick={handleSubmit}><Check className="h-4 w-4" />{saving ? '保存中...' : '保存'}</Button></>} />
    <MarkdownEditorPanel title="正文内容" value={form.content} onChange={(content) => setForm((previous) => ({ ...previous, content }))} fullscreen={fullscreen} onFullscreenChange={setFullscreen} fullscreenTitle={form.title || '未命名杂谈'} onSave={() => void handleSubmit()} saving={saving} metadata={<>
